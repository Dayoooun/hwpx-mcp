...
- **`update_paragraph_text` 로 글자 모양이 섞인 문단을 통째로 바꾸면 뒤쪽이 굵게 바뀌던
  ...
  를 씁니다. 한/글 2020 이 저장한 원본 83건의 첫 섞인 문단에서 0.3.3 은 3건, 0.3.4 는 4건,
  이 버전은 82건이 제대로 바뀌었습니다(1건은 아래 글상자 문단이라 거부).
...
- **글이 전부 글상자 안에 있는 문단을 바꾸면 성공이라 답하고 글상자 밖에 쓰던 문제.** 새 글이
  글상자 옆에 작게 따로 찍혔습니다(한/글 2024 에서 확인). 이제 이런 문단은 거부하고 글상자
  안 글은 `replace_text` 로 바꾸라고 알려 줍니다.

한/글 원본 200건에서 문단 통째 교체를 재면, 파서가 읽은 글과 문단 자기 글이 같은 문단은
0.3.4 가 144건 중 138건, 이 버전이 144건 모두 제자리에 들어갔습니다. 둘이 다른 문단 28건은
0.3.4 가 모두 어긋났고, 이 버전은 9건 제자리, 17건 거부, 2건은 0.3.4 와 같습니다.

### 알려진 제한

- 머리말·꼬리말을 품은 첫 문단을 통째로 바꾸면, 그 문단의 머리말 안 글과 바깥 글 중 바깥 글이
  바뀝니다(0.3.3 은 머리말 글을 지웠습니다). 머리말 글을 바꾸려면 `replace_text` 를 씁니다.
